...
function useAlbums() {
  ...
  const load = async (reset = false): Promise<Album[]> => {
    ...
    const j = await fetchJSON<{ items: Album[]; nextAfter: number | null }>(`/api/admin/gallery/list?limit=50${q}`);
...
export default function AdminGalleryPage() {
  ...
  async function createAlbum() {
    setSaving(true);
    try {
      const j = await fetchJSON<{ id: string; slug: string }>('/api/admin/gallery/create', {
        method: 'POST',
  ...
  async function openAlbum(a: Album) {
    ...
    try {
      const j = await fetchJSON<{ items: MediaItem[] }>(`/api/admin/gallery/media-list?groupId=${a.id}&limit=200`);
      setMedia(j.items || []);
      setIndexUrl(null);
    } catch (e: unknown) {
  ...
  async function uploadFiles(files: FileList) {
    ...
    try {
      ...
      for (const file of list) {
        ...
        await fetchJSON('/api/admin/gallery/upload-media', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            albumId: active.id,
  ...
  async function removeMedia(m: MediaItem) {
    ...
    await fetchJSON('/api/admin/gallery/delete-media', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function renameMedia(m: MediaItem, title: string) {
    await fetchJSON('/api/admin/gallery/update-media', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <>
      <Head><title>Admin • Gallery</title></Head>

      <div className="mx-auto max-w-7xl px-4 py-8">
        <h1 className="text-3xl md:text-4xl font-extrabold">Admin / Gallery</h1>
        <p className="text-gray-600 mt-1">Create an album (title + description), then upload images/videos under it. Toggle publish when ready.</p>

        {/* Create album */}
        <div className="mt-6 rounded-2xl border p-4 bg-white">
          <h2 className="font-semibold text-lg">Create New Album</h2>
          <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
            <input className="rounded-xl border px-3 py-2" placeholder="Title" value={form.title} onChange={(e) => setForm(v => ({ ...v, title: e.target.value }))} />
            <input className="rounded-xl border px-3 py-2 md:col-span-2" placeholder="Description" value={form.description} onChange={(e) => setForm(v => ({ ...v, description: e.target.value }))} />
          </div>
          <div className="mt-3 flex items-center gap-3">
            ...
            <button disabled={!form.title || saving} onClick={createAlbum} className="rounded-xl px-4 py-2 bg-black text-white disabled:opacity-50">Create</button>
          </div>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div id="album-editor" className="rounded-2xl border bg-white">
            <div className="p-4 border-b flex items-center justify-between">
              <h2 className="font-semibold">Album Editor</h2>
              {active ? <button onClick={deleteAlbum} className="text-red-600 hover:underline">Delete album</button> : null}
            </div>

            {!active ? (
              <div className="p-4 text-gray-500">Select an album to edit and upload media.</div>
            ) : (
              <div className="p-4 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <label className="block">
                    ...
                    <input className="mt-1 w-full rounded-xl border px-3 py-2" value={active.title} onChange={(e) => updateAlbum({ title: e.target.value }) as unknown} />
                  </label>
                  <label className="block">
                  ...
                  <label className="block md:col-span-2">
                    ...
                    <textarea className="mt-1 w-full rounded-xl border px-3 py-2" rows={3} value={active.description} onChange={(e) => updateAlbum({ description: e.target.value }) as unknown} />
                  </label>
                </div>
                ...
                {indexUrl ? (
                  <div className="rounded-xl border border-yellow-300 bg-yellow-50 p-3 text-sm text-yellow-900">
                    This album view needs a Firestore composite index.
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                      {media.map((m) => (
                        <div key={m.id} className="relative group rounded-xl overflow-hidden border bg-gray-50" title={m.title}>
                          ...
                          <input className="w-full border-t px-2 py-2 text-sm" value={m.title} onChange={(e) => renameMedia(m, e.target.value)} />
                        </div>
                      ))}
                    </div>
                  )}
