...
export const config = {
  api: {
    bodyParser: {
      // base64 has ~33% overhead; 50mb keeps you safe for ~37mb binary
      sizeLimit: '100mb',
    },
  },
};
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!filename || !contentType || !data) {
      return res
        .status(400)
        .json({ ok: false, error: 'Missing filename/contentType/data' });
    }
    ...
    if (!adminStorage || !adminStorage.name) {
      return res.status(500).json({
        ok: false,
        error:
          'Storage bucket is not configured. Set FIREBASE_STORAGE_BUCKET or FIREBASE_PROJECT_ID.',
      });
    }
    ...
    if (!buffer?.length) {
      return res.status(400).json({ ok: false, error: 'Empty file buffer' });
    }

    // Compose storage path
    const uidPart = safe(
      ((session.user as unknown)?.email || 'admin').split('@')[0] || 'admin',
    );
    const id = uuidv4();
    ...
    const kind = String(contentType).toLowerCase().startsWith('video/')
      ? 'video'
      : 'image';
    ...
    return res.status(200).json({
      ...
      id: ref.id,
      doc: { id: ref.id, ...doc },
      // Return bucket/object meta only in dev
