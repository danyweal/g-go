// pages/api/admin/news/upload-media.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const {
      newsId, type, title = '', filename: rawFilename, contentType, data,
      thumbData, thumbContentType = 'image/jpeg', setAsPrimary = false,
    } = req.body || {};

    if (!newsId) return res.status(400).json({ ok: false, error: 'missing_newsId' });
    ...
    const newsRef = adminDb.collection('cms').doc('root').collection('news').doc(String(newsId));
    const eventDoc = await newsRef.get();
    ...
    const objectPath = `cms/root/news/${newsId}/${filename}`;
    ...
    if (thumbData) {
      ...
      const thumbPath = `cms/root/news/${newsId}/${thumbName}`;
    ...
    console.error('POST /api/admin/news/upload-media:', err);
