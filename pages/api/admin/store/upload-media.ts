// pages/api/admin/store/upload-media.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  note = ''
    ...
    const albumRef = adminDb.collection('cms').doc('root').collection('stores').doc(String(albumId));
    ...
    const objectPath = `cms/root/store/${albumId}/${filename}`;
    ...
    if (thumbData) {
      ...
      const thumbPath = `cms/root/store/${albumId}/${thumbName}`;
    ...
    console.error('POST /api/admin/store/upload-media:', err);
