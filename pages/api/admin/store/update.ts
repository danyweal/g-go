// pages/api/admin/store/update.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const { id, name, title, address, description, published, primaryMediaId } = req.body || {};
    if (!id) return res.status(400).json({ ok: false, error: 'missing_id' });

    const ref = adminDb.collection(CMS[0]).doc(CMS[1]).collection('stores').doc(String(id));
