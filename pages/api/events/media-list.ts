// pages/api/events/media-list.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 1) Primary source: /cms/root/events/{groupId}/media
    const eventsMediaCol = adminDb
      .collection(CMS[0]).doc(CMS[1])
      .collection('events').doc(groupId)
      .collection('media');

    let snap = await eventsMediaCol.limit(lim).get();

    // 2) Fallback to legacy: /cms/root/media where groupId == {groupId}
    if (snap.empty) {
    ...
    // Oldest -> newest (match gallery behavior)
    items.sort((a, b) => (a.createdAtMillis ?? 0) - (b.createdAtMillis ?? 0));

    res.setHeader('Cache-Control', 's-maxage=60, stale-while-revalidate=300');
    return res.status(200).json({ ok: true, items });
  } catch (err: unknown) {
    console.error('GET /api/events/media-list:', err);
    return res
      .status(500)
